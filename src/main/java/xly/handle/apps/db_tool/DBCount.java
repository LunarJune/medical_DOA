...
package xly.handle.apps.db_tool;

import net.cnri.util.StreamTable;
import xly.handle.hdllib.*;
import xly.handle.server.*;
import xly.handle.server.bdbje.BDBJEHandleStorage;

public class DBCount {
    public static void main(String argv[]) throws Exception {
        if (argv == null || argv.length < 1) {
            System.err.println("usage: java xly.handle.apps.db_tool.DBCount <server-directory>");
            return;
        }
