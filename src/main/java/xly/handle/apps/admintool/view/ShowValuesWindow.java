...
public class ShowValuesWindow extends JFrame implements ActionListener, MouseListener, ViewConstants {
    ...
    private void addDigestAndSignatureForAllValues() {
        try {
            ...
            HandleValue digestValue = xly.handle.hdllib.HandleSignature.createDigestsValue(getNextUnusedIndex(400), handle, values);
            ...
            @SuppressWarnings("deprecation")
            HandleValue sigValue = xly.handle.hdllib.HandleSignature.createSignatureValue(getNextUnusedIndex(digestValue.getIndex() + 1), handle, signer, null, localSigInfo.getPrivateKey(), digestValue);
