...
package xly.handle.server;

import net.cnri.util.StreamTable;
import net.cnri.util.StreamVector;
import xly.handle.apps.servlet_proxy.DefaultServlet;
import xly.handle.apps.servlet_proxy.HDLProxy;
import xly.handle.apps.servlet_proxy.ResponseHeaderFilter;
import xly.handle.hdllib.*;
import xly.handle.server.dns.DnsConfiguration;
import xly.handle.server.replication.ReplicationDaemon;
...
import xly.handle.server.servletcontainer.HandleApiErrorHandler;
import xly.handle.server.servletcontainer.HandleAuthorizationEnabledSessionHandler;
import xly.handle.server.servletcontainer.auth.StandardHandleAuthenticationFilter;
import xly.handle.server.servletcontainer.servlets.DisallowHttpZeroDotNineFilter;
import xly.handle.server.servletcontainer.servlets.HandleJsonRestApiServlet;
import xly.handle.server.servletcontainer.servlets.NativeServlet;
import xly.handle.server.servletcontainer.servlets.PrefixesServlet;
import xly.handle.server.servletcontainer.servlets.SessionsServlet;
import xly.handle.server.servletcontainer.servlets.SiteServlet;
import xly.handle.server.servletcontainer.servlets.UncaughtExceptionsFilter;
import xly.handle.server.servletcontainer.servlets.UnknownApiServlet;
import xly.handle.server.servletcontainer.servlets.VerificationsServlet;
...
public class Main {
    ...
    public static void main(String argv[]) {
        System.out.println("Handle.Net Server Software version " + Version.version);
        args = argv;
    ...
    public void dumpFromPrimary(boolean deleteAll, SiteInfo[] sites) throws Exception {
        ...
        if (!(server instanceof HandleServer)) throw new HandleException(HandleException.SERVER_ERROR, "server is of a type that cannot be replicated");
    ...
    private void initEmbeddedJetty() throws Exception {
        ...
        jettyConfig.addContextAttribute("xly.handle.server.Main", this);
        if (server instanceof HandleServer) {
            jettyConfig.addContextAttribute("xly.handle.server.HandleServer", server);
        }
        jettyConfig.addSystemClass("xly.handle.hdllib.");
        jettyConfig.addSystemClass("xly.handle.server.");
        jettyConfig.addSystemClass("com.google.gson.");
        jettyConfig.setBaseDir(serverDir);
        jettyConfig.setEnableGzip(true);
    ...
    private void configureDefaultRootWebApp(EmbeddedJettyConfig jettyConfig, StreamTable serverHttpConfig, boolean enableTrace) throws Exception {
        ...
        if (serverHttpConfig != null && serverHttpConfig.containsKey("session_timeout_seconds")) {
            ...
            sessionHandler.setMaxInactiveInterval(sessionTimeoutSeconds);
        }

        context.setAttribute("xly.handle.server.Main", this);
        context.setAttribute(org.eclipse.jetty.server.session.SessionHandler.class.getName(), sessionHandler);
        if (server instanceof HandleServer) {
            context.setAttribute("xly.handle.server.HandleServer", server);
        }
        ...
        if (serverHttpConfig != null && serverHttpConfig.get("headers") != null) {
            ...
            // do not re-run this one on async
    ...
    private void configureEmbeddedJettyConnector(EmbeddedJettyConfig jettyConfig, StreamTable connectorConfigTable) throws UnknownHostException {
        ...
        if (webserverHttpAddressString != null) httpListenAddress = InetAddress.getByName(String.valueOf(webserverHttpAddressString));
        ...
        connectorConfig.setHttpsClientAuth(connectorConfigTable.getStr("https_client_auth", "renegotiate"));
