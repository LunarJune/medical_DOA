...
package xly.handle.server.servletcontainer.auth;
...
import xly.handle.hdllib.ResolutionRequest;
import xly.handle.hdllib.ResolutionResponse;
import xly.handle.hdllib.SecretKeyAuthenticationInfo;
import xly.handle.hdllib.Util;
import xly.handle.hdllib.ValueReference;
import xly.handle.hdllib.VerifyAuthRequest;
import xly.handle.hdllib.VerifyAuthResponse;
import xly.handle.server.servletcontainer.HandleServerInterface;
import xly.handle.server.servletcontainer.support.PreAuthenticatedAuthenticationInfo;
import xly.handle.util.X509HSTrustManager;
...
public class StandardHandleAuthenticator {
    ...
    private boolean checkSecretKey(ValueReference val, byte[] password) {
        ...
        HandleServerInterface handleServer = (xly.handle.server.servletcontainer.HandleServerInterface) servletContext.getAttribute("xly.handle.server.HandleServer");
    ...
    // only specify TrustManager once
    private X509HSTrustManager getTrustManager(ServletContext servletContext) {
        X509HSTrustManager trustManager = (X509HSTrustManager) servletContext.getAttribute(X509HSTrustManager.class.getName());
        if (trustManager == null) {
            synchronized (servletContext) {
                trustManager = (X509HSTrustManager) servletContext.getAttribute(X509HSTrustManager.class.getName());
                if (trustManager == null) {
                    trustManager = new X509HSTrustManager(getHandleResolverEvenIfInServer(servletContext));
                    servletContext.setAttribute(X509HSTrustManager.class.getName(), trustManager);
                }
            }
        }
        return trustManager;
    ...
    private AuthenticationInfoWithId authenticateViaClientSideCert() {
        X509Certificate[] certs = extractCertificates(request);
        if (certs == null || certs.length == 0) return null;
        X509Certificate cert = certs[0];
        try {
            String authType = cert.getPublicKey().getAlgorithm();
            getTrustManager(request.getServletContext()).checkClientTrusted(certs, authType);
        } catch (Exception e) {
            String message = e.getMessage();
            if (e.getCause() != null && e.getCause().getMessage() != null) {
                message += ": " + e.getCause().getMessage();
            }
            authResp.addError(message);
            return null;
        }
    ...
    public static X509Certificate[] extractCertificates(HttpServletRequest request) {
