...
import xly.handle.server.servletcontainer.auth.AuthenticationInfoWithId;
import xly.handle.server.servletcontainer.auth.AuthenticationResponse;
import xly.handle.server.servletcontainer.auth.HandleAuthenticationStatus;
import xly.handle.server.servletcontainer.auth.HandleAuthorizationHeader;
import xly.handle.server.servletcontainer.auth.StandardHandleAuthenticationFilter;
import xly.handle.server.servletcontainer.auth.StandardHandleAuthenticator;
...
public class SessionsServlet extends BaseHandleRequestProcessingServlet {
    ...
    public void init() throws ServletException {
        super.init();
        sessionManager = (SessionHandler) getServletContext().getAttribute(SessionHandler.class.getName());
    }
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (BaseHandleRequestProcessingServlet.hasJsonEntity(req) && !wrapsJsonParameterServletReq(req)) {
            try {
                req = new JsonParameterServletReq(req);
        ...
        HandleAuthorizationHeader handleAuthHeader = HandleAuthorizationHeader.fromHeaderAndParameters(req.getHeader("Authorization"), req);
        if (StandardHandleAuthenticationFilter.requestTlsRenegotiationReturnTerminal(handleAuthHeader, req, resp)) return;
    ...
    private boolean wrapsJsonParameterServletReq(HttpServletRequest req) {
        if (req instanceof JsonParameterServletReq) return true;
        if (!(req instanceof HttpServletRequestWrapper)) return false;
        return ((HttpServletRequestWrapper)req).isWrapperFor(JsonParameterServletReq.class);
    }
