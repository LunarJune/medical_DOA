...
public abstract class Encoder {
    ...
    public static final byte[] readByteArray(byte buf[], int offset) throws HandleException {
        ...
        if (len < 0 || len > Common.MAX_ARRAY_SIZE) throw new HandleException(HandleException.MESSAGE_FORMAT_ERROR, MSG_INVALID_ARRAY_SIZE);
        byte a[] = new byte[len];
    ...
    public static void decodeSiteInfoRecord(byte data[], int offset, SiteInfo site) throws HandleException {
        ...
        if (numAtts < 0 || numAtts > Common.MAX_ARRAY_SIZE) throw new HandleException(HandleException.MESSAGE_FORMAT_ERROR, MSG_INVALID_ARRAY_SIZE);
        ...
        if (numServers < 0 || numServers > Common.MAX_ARRAY_SIZE) throw new HandleException(HandleException.MESSAGE_FORMAT_ERROR, MSG_INVALID_ARRAY_SIZE);
        ...
        for (int i = 0; i < site.servers.length; i++) {
            ...
            if (numIntf < 0 || numIntf > Common.MAX_ARRAY_SIZE) throw new HandleException(HandleException.MESSAGE_FORMAT_ERROR, MSG_INVALID_ARRAY_SIZE);
        ...
        if (offset < data.length) throw new HandleException(HandleException.MESSAGE_FORMAT_ERROR, "Unexpected data remaining after decoding");
    }
    ...
    public static void decodeAdminRecord(byte data[], int offset, AdminRecord admin) throws HandleException {
        ...
        if (!admin.legacyByteLength && offset < data.length) throw new HandleException(HandleException.MESSAGE_FORMAT_ERROR, "Unexpected data remaining after decoding");
    }
    ...
    public static final byte[] encodeMessage(AbstractMessage msg) throws HandleException {
        ...
        if (buf == null) throw new HandleException(HandleException.INTERNAL_ERROR, "Encoder.encodeMessage() not implemented for " + "type: " + msg.opCode);
        return buf;
    }
    ...
    public static final void decodeEnvelope(byte udpPkt[], MessageEnvelope msgEnv) throws HandleException {
        if (udpPkt == null || udpPkt.length < Common.MESSAGE_ENVELOPE_SIZE) throw new HandleException(HandleException.MESSAGE_FORMAT_ERROR, "Invalid message envelope");
        ...
        if (msgEnv.messageLength > Common.MAX_MESSAGE_LENGTH || msgEnv.messageLength < 0) throw new HandleException(HandleException.MESSAGE_FORMAT_ERROR, "Invalid message length: " + msgEnv.messageLength);
    }
    ...
    public static ResolutionResponse decodeResolutionResponse(byte msg[], int offset, @SuppressWarnings("unused") MessageEnvelope env) throws HandleException {
        ...
        if (handleLen < 0 || handleLen > Common.MAX_HANDLE_LENGTH) throw new HandleException(HandleException.MESSAGE_FORMAT_ERROR, "Invalid handle length: " + handleLen);
        ...
        if (numValues < 0 || numValues > Common.MAX_HANDLE_VALUES) throw new HandleException(HandleException.MESSAGE_FORMAT_ERROR, "Invalid number of values: " + numValues);
    ...
    public static final ValueReference[] decodeValueReferenceList(byte buf[], int offset) throws HandleException {
        ...
        if (numValues < 0 || numValues > Common.MAX_ARRAY_SIZE) throw new HandleException(HandleException.MESSAGE_FORMAT_ERROR, MSG_INVALID_ARRAY_SIZE);
        ...
        if (offset < buf.length) throw new HandleException(HandleException.MESSAGE_FORMAT_ERROR, "Unexpected data remaining after decoding");
